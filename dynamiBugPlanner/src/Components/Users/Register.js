...
export default function Register({api}) {
    ...
    const postData = async () => {
        ...
        result
          .json()
          .then((data) => { sessionStorage.setItem('user', JSON.stringify(data)); window.location.href = '/'; })
          .catch((err) => {
        ...
        <form className="container CFcontainer" onSubmit={(e) => { e.preventDefault(); postData(); }}>
            ...
            <div className="row">
                ...
                <input
                    ...
                    required
                />
                <span id="userPassword2Warning" hidden>Password doesn't match</span>
            </div>
            <Captcha />
            <div className="row">
                <input className="CFbutton CFinput btn btn-primary" type="submit" value="Register" />
            </div>
        </form>
    );
}